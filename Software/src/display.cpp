...
void HM_Display::set_rotary_scale() {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_helvB08_tf);
    sprintf(output,SELECT_ROTARY[lingo]);
    x_pos = CenterPosX(output, 5, 128);
    u8g2.setCursor(x_pos, 10);  u8g2.print(output);
    u8g2.setCursor(0, 35);      u8g2.print(START[lingo]); u8g2.print(":");
    u8g2.setCursor(87, 35);     u8g2.print("HW-040");
    u8g2.setCursor(0, 47);      u8g2.print(STOP[lingo]);  u8g2.print(":");
    u8g2.setCursor(88, 47);     u8g2.print("KY-040");
    u8g2.sendBuffer();
  #endif
  #if DISPLAY_TYPE == 993 or DISPLAY_TYPE == 999
    ...
    x_pos = tft.width() / 2;
    tft.drawString(SELECT_ROTARY[lingo], x_pos, 11);
    tft.drawLine(0, 38, 320, 38, TEXT);
    tft.setFreeFont(F12B[font_typ]);
    tft.setCursor(5, 70);
    tft.print(START[lingo]); tft.print(":");
    sprintf(output,"%s", "HW-040");
    tft.setCursor(tft.width() - 5 - tft.textWidth(output), 70);
    tft.print(output);
    tft.setCursor(5, 100);
    tft.print(STOP[lingo]); tft.print(":");
    sprintf(output,"%s", "KY-040");
    tft.setCursor(tft.width() - 5 - tft.textWidth(output), 100);
    tft.print(output);  
  #endif  
}
...
void HM_Display::no_scale() {
    ...
    tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - x_y_h_w_b[2]/2 - x_y_h_w_b[2] * a);
    tft.print(output);
    sprintf(output, "%s", SCALE_1[lingo]);
    calculateStringMetrics(F28B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
    tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] + x_y_h_w_b[2]/2 + x_y_h_w_b[2] * a);
    tft.print(output);
  #endif
...
void HM_Display::process_manualmode() {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_courB24_tf);
...
void HM_Display::process_automatic(int a) {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 1) { //clear display
      u8g2.clearBuffer();
      u8g2.sendBuffer();
    }
    if (a == 2) { //connection failed
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
    ...
    if (a == 2) { //connection failed
      ...
      sprintf(output, "%s", CONNECTION[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - x_y_h_w_b[2]/2 - x_y_h_w_b[2] * a);
      tft.print(output);
      sprintf(output, "%s", FAILED[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] + x_y_h_w_b[2]/2 + x_y_h_w_b[2] * a);
      tft.print(output);
      tft.setTextColor(TEXT);
    ...
    if (a == 3) { //init turntable nok
      ...
      tft.setTextColor(TFT_RED);
      tft.setFreeFont(F18B[font_typ]);
      sprintf(output, "%s", INIT_TURNTABLE[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - x_y_h_w_b[2]/2 - x_y_h_w_b[2] * a);
      tft.print(output);
      sprintf(output, "%s", NOKAY[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] + x_y_h_w_b[2]/2 + x_y_h_w_b[2] * a);
      tft.print(output);
      tft.setTextColor(TEXT);
    ...
    if (a == 4) { //close dropprotection
      ...
      tft.setTextColor(TFT_RED);
      tft.setFreeFont(F16B[font_typ]);
      sprintf(output, "%s", CLOSE_DRIP_DP[lingo]);
      calculateStringMetrics(F16B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - 3 * x_y_h_w_b[2]/2 - 3 * x_y_h_w_b[2] * a);
      tft.print(output);
      sprintf(output, "%s", PROTECTION_WAIT_DP[lingo]);
      calculateStringMetrics(F16B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - x_y_h_w_b[2]/2 - x_y_h_w_b[2] * a);
      tft.print(output);
      sprintf(output, "%s", TIME_READING_DP[lingo]);
      calculateStringMetrics(F16B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] + x_y_h_w_b[2]/2 + x_y_h_w_b[2] * a);
      tft.print(output);
      sprintf(output, "%s", FAILED_DP[lingo]);
      calculateStringMetrics(F16B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] + 3 * x_y_h_w_b[2]/2 + 3 * x_y_h_w_b[2] * a);
      tft.print(output);
      tft.setTextColor(TEXT);
...
void HM_Display::process_setup() {
    ...
    if (menuitem != menuitem_old) {
      ...
      if (menu_rotation == 1) {
        ...
        x_pos = CenterPosX(menuitems[posmenu[numItems-1]], 6, 128); //ein Zeichen ist etwa 6 Pixel breit
        u8g2.setCursor(x_pos,12);  
        u8g2.print(menuitems[posmenu[numItems-1]]);
      }
    ...
    if (menuitem != menuitem_old) {
      ...
      for (int a = 0; a < 4; a++) {
        ...
        sprintf(output, "%s", menuitems[posmenu[(numItems-1-a)]]);
...
void HM_Display::setup_servoWinkel() {
  if (((pos >= 6 and pos != numItems - 1) or (pos_old == 6 and pos == 5)) and pos_old != pos) {
    pos_old = pos;
    #if DISPLAY_TYPE == 993 or DISPLAY_TYPE == 999
      clear_tft = 1;
    #endif
  ...
  if (pos >= 6 and pos != numItems - 1) {
    ...
    while(x <= j) {
      if (x < j) {
        u8g2.setCursor(10, (x+1-offset) * y_offset);
        u8g2.print(menuitems[x]);
        switch (x) {
          case 0: sprintf(output,"%5s", (servo_live==false?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 1: sprintf(output,"%4d°", angle_min);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 2: sprintf(output,"%5d", fullmode);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 3: sprintf(output,"%4.0fg", finedos_weight[fullmode-1]);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 4: sprintf(output,"%4d°", angle_fine[fullmode-1]);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 5: sprintf(output,"%4d°", angle_max[fullmode-1]);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 6: sprintf(output,"%5s", (squee_tap_left==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 7: sprintf(output,"%5s", (servo_expanded==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
        }
        if (x == pos and change_value == false) {
          u8g2.setCursor(1, 2+((x+1-offset) * y_offset));
          u8g2.print("*");
        }
        else if (x == pos and change_value == true) {
          u8g2.setCursor(1, ((x+1-offset) * y_offset));
          u8g2.print("-");
        }
      }
      else {
        if (x == pos) { 
          u8g2.setCursor(1, 2+((7 * y_offset) + 5));
          u8g2.print("*");
        }
        u8g2.setCursor(10, (7 * y_offset) + 5);
        u8g2.print(menuitems[numItems - 1]);
      }
      x++;
    }
    u8g2.sendBuffer();
    ...
    while(y <= j) {
      tft.setTextColor(TEXT);
      if (y == pos and change_value == false) {
        tft.setTextColor(MARKER);
      }
      if (y < j) {
        tft.setCursor(5, 27+((y+1-offset) * y_offset_tft));
        tft.print(menuitems[y]);
        tft.setTextDatum(BR_DATUM);
        tft.setTextColor(BACKGROUND);
        switch (y) {
          case 0: sprintf(output,"%s", servo_live==false?OFF[lingo]:ON[lingo]);
                  if (value_old != servo_live and change_value == true and y == pos) {
                    del_text = servo_live==true?OFF[lingo]:ON[lingo];
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 27+((y+1) * y_offset_tft));
                    tft.print(del_text);                    
                    value_old = servo_live;
                  }
                  break;
          case 1: sprintf(output,"%d°", angle_min);
                  if (value_old != angle_min and change_value == true and y == pos) {
                    sprintf(del_text_1,"%d°", value_old);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1) * y_offset_tft));
                    tft.print(del_text_1);
                    value_old = angle_min;
                  }
                  break;
          case 2: sprintf(output,"%d", fullmode);
                  if (value_old != fullmode and change_value == true and y == pos) {
                    sprintf(del_text_1,"%d", value_old);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1) * y_offset_tft));
                    tft.print(del_text_1);
                    ...
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+2) * y_offset_tft));
                    tft.print(del_text_1);
                    sprintf(del_text_1,"%d°", angle_fine[value_old-1]);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+3) * y_offset_tft));
                    tft.print(del_text_1);
                    sprintf(del_text_1,"%d°", angle_max[value_old-1]);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+4) * y_offset_tft));
                    tft.print(del_text_1);
                    //value_old setzen
                    value_old = fullmode;
                  }
          ...
          case 3: sprintf(output,"%.0fg", finedos_weight[fullmode-1]);
                  if (value_old * 1.00 != finedos_weight[fullmode-1] and change_value == true and y == pos) {
                    sprintf(del_text_1,"%dg", value_old);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1) * y_offset_tft));
                    tft.print(del_text_1);
          ...
          case 4: sprintf(output,"%d°", angle_fine[fullmode-1]);
                  if (value_old != angle_fine[fullmode-1] and change_value == true and y == pos) {
                    sprintf(del_text_1,"%d°", value_old);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1) * y_offset_tft));
                    tft.print(del_text_1);
          ...
          case 5: sprintf(output,"%d°", angle_max[fullmode-1]);
                  if (value_old != angle_max[fullmode-1] and change_value == true and y == pos) {
                    sprintf(del_text_1,"%d°", value_old);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1) * y_offset_tft));
                    tft.print(del_text_1);
                  ...
                  break;
          case 6: sprintf(output,"%s", squee_tap_left==0?OFF[lingo]:ON[lingo]);
                  if (value_old != squee_tap_left and change_value == true and y == pos) {
                    del_text = squee_tap_left==1?OFF[lingo]:ON[lingo];
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 27+(y * y_offset_tft));
                    tft.print(del_text);                    
                    value_old = squee_tap_left;
                  }
                  break;
          case 7: sprintf(output,"%s", servo_expanded==false?OFF[lingo]:ON[lingo]);
                  if (value_old != servo_expanded and change_value == true and y == pos) {
                    del_text = servo_expanded==true?OFF[lingo]:ON[lingo];
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 27+((y-1) * y_offset_tft));
                    tft.print(del_text);                    
                    value_old = servo_expanded;
                  }
                  break;
        }
        if (y == pos) {
        ...
        tft.setCursor(tft.width() - 5 - tft.textWidth(output), 27+((y+1-offset) * y_offset_tft));
        tft.print(output);
      }
      else {
        tft.setCursor(5, 35+(7 * y_offset_tft));
        tft.print(menuitems[numItems - 1]);
      }
      y++;
    }
  #endif
...
void HM_Display::setup_automatik() {
  if (((pos >= 6 and pos != numItems - 1) or (pos_old == 6 and pos == 5)) and pos_old != pos) {
    pos_old = pos;
    #if DISPLAY_TYPE == 993 or DISPLAY_TYPE == 999
      clear_tft = 1;
  ...
  if (pos >= 6 and pos != numItems - 1) {
    ...
    while(x <= j) {
      if (x < j) {
        ...
        u8g2.print(menuitems[x]);
        switch (x) {
          case 0: sprintf(output,"%5s", (autostart==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 1: if (jartolerance > 0) {
                    if (jartolerance > 9) {sprintf(output," %c%2dg", 177, jartolerance);}
                    else {sprintf(output,"  %c%1dg", 177, jartolerance);}
                  }
                  else {sprintf(output,"%4dg", jartolerance);}
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 2: sprintf(output,"%4dg", correction);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 3: sprintf(output,"%5s", (autocorrection==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 4: sprintf(output,"%4dg", overfill_gr);
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 5: if (init_weight_f > 0) {sprintf(output,"%4dg", init_weight_f);}
                  else {sprintf(output,"%5s", OFF[lingo]);  }
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 6: sprintf(output,"%5s", (wait_befor_fill==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
        }
      ...
      if (y < j) {
        ...
        switch (y) {
          case 0: sprintf(output,"%s", autostart==false?OFF[lingo]:ON[lingo]);
                  if (value_old != autostart and change_value == true and y == pos) {
                    del_text = autostart==true?OFF[lingo]:ON[lingo];
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 27+((y+1-offset) * y_offset_tft));
                    tft.print(del_text);
                    value_old = autostart;
                  }
                  break;
          case 1: if (jartolerance > 0) {
                  ...
                  if (value_old != jartolerance and change_value == true and y == pos) {
                    if (value_old > 0) {sprintf(del_text_1,"±%ig", value_old);}
                    else {sprintf(del_text_1,"%ig", value_old);}
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1-offset) * y_offset_tft));
                    tft.print(del_text_1);
                    value_old = jartolerance;
                  }
                  break;
          case 2: sprintf(output,"%dg", correction);
                  if (value_old != correction and change_value == true and y == pos) {
          ...
          case 3: sprintf(output,"%s", autocorrection==false?OFF[lingo]:ON[lingo]);
                  if (value_old != autocorrection and change_value == true and y == pos) {
          ...
          case 4: sprintf(output,"%dg", overfill_gr);
                  if (value_old != overfill_gr and change_value == true and y == pos) {
                    sprintf(del_text_1,"%ig", value_old);
                    tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 27+((y+1-offset) * y_offset_tft));
                    tft.print(del_text_1);
          ...
          case 5: if (init_weight_f > 0) {
                  ...
                  if (value_old != init_weight_f and change_value == true and y == pos) {
                    if (value_old > 0) {sprintf(del_text_1,"%ig", value_old);}
          ...
          case 6: sprintf(output,"%s", wait_befor_fill==false?OFF[lingo]:ON[lingo]);
                  if (value_old != wait_befor_fill and change_value == true and y == pos) {
...
void HM_Display::setup_parameter() {
  int i = 0;
  int j = numItems-1;
  int offset = 0;
    ...
    if (numItems > 6) {
      menuitems.erase(std::remove_if(menuitems.begin(), menuitems.end(), 
          [](const char* item) { 
              return (item == COLORSCHEME[lingo] || item == MARKER_COLOR[lingo] || item == FONT[lingo]); 
          }), menuitems.end());
      auto it = std::find(menuitems.begin(), menuitems.end(), SAVE[lingo]);
      if (it != menuitems.end()) {
          menuitems.erase(it);
          menuitems.push_back(SAVE[lingo]);
      }
  ...
  if (numItems > 6) {
    ...
    while(x <= j) {
      if (x < j) {
        ...
        u8g2.print(menuitems[x]);
        switch (x) {
          case 0: sprintf(output,"%5s", (buzzermode==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 1: sprintf(output,"%5s", (ledmode==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 2: sprintf(output,"%5s", (showlogo==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 3: sprintf(output,"%5s", (showcredits==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
          case 4: sprintf(output,"%5s", (menu_rotation==0?OFF[lingo]:ON[lingo]));
                  u8g2.setCursor(95, (x+1-offset) * y_offset);
                  u8g2.print(output);
                  break;
        }
      ...
      if (y < j) {
        ...
        switch (y) {
          case 0: sprintf(output,"%s", buzzermode==false?OFF[lingo]:ON[lingo]);
                  if (value_old != buzzermode and change_value == true and y == pos) {
          ...
          case 1: sprintf(output,"%s", ledmode==false?OFF[lingo]:ON[lingo]);
                  if (value_old != ledmode and change_value == true and y == pos) {
          ...
          case 2: sprintf(output,"%s", showlogo==false?OFF[lingo]:ON[lingo]);
                  if (value_old != showlogo and change_value == true and y == pos) {
          ...
          case 3: sprintf(output,"%s", showcredits==false?OFF[lingo]:ON[lingo]);
                  if (value_old != showcredits and change_value == true and y == pos) {
          ...
          case 4: sprintf(output,"%s", menu_rotation==false?OFF[lingo]:ON[lingo]);
                  if (value_old != menu_rotation and change_value == true and y == pos) {
          ...
          case 5: sprintf(output,"%s", color_scheme==false?DARK[lingo]:LIGHT[lingo]);
                  if (value_old != color_scheme and change_value == true and y == pos) {
          ...
          case 6: sprintf(output,"");
                  if (value_old != color_marker and change_value == true and y == pos) {
                    change_marker = true;
          ...
          case 7: sprintf(output,"%s", font_used[font_typ]);
                  if (value_old != font_typ and change_value == true and y == pos) {
                    if (value_old >= 0) {draw_frame = true;}
                    value_old = font_typ;
                  }
                  break;
        }
...
void HM_Display::setup_language() {
  int i;
  int j;
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    u8g2.setFont(u8g2_font_courB08_tf);
    ...
    i = 0;
    j;
    if (numItems < 6) {j = numItems;}
    else {j = 6;}
    if (pos >= 6 and pos != numItems) {i = pos - 5; j = pos + 1;}
    else if (pos == numItems and numItems > 6) {i = pos - 6; j = pos;}
    for (i; i < j; i++) {
      if (i == lingo) {
    ...
    if ((pos < 6 and numItems > 6) or (numItems <= 6 and pos != numItems)) {
      u8g2.setCursor(1, 10+(pos*y_offset)); u8g2.print("*");
    }
    else if (pos < numItems  and numItems > 6) {
    ...
    if (((pos >= 6 and pos != numItems) or (pos_old == 6 and pos == 5)) and pos_old != pos) {
      tft.fillRect(0, 31, 320, 175, BACKGROUND);
      pos_old = pos;
    }
    i = 0;
    ...
    tft.setTextColor(TEXT);
    if (pos == numItems) {tft.setTextColor(MARKER);}
    tft.setCursor(5, 35+(7 * y_offset_tft));
    tft.print(SAVE[lingo]);
  #endif
}

void HM_Display::setup_clear_prefs() {
  int j;
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    u8g2.setFont(u8g2_font_courB08_tf);
    u8g2.clearBuffer();
    ...
    while(j < numItems) {
      if (j == pos) {
        if (j < numItems-1) {
          u8g2.setCursor(0, (j+1) * y_offset + 2);
        }
        else {
          u8g2.setCursor(0, (7 * y_offset) + 9);
        }
        u8g2.print("*");
      }
      if (j < numItems-1) {
        u8g2.setCursor(10, (j+1) * y_offset);
        u8g2.print(menuitems[j]);
      }
      else {
        u8g2.setCursor(10, (7 * y_offset) + 7);
        u8g2.print(menuitems[j]);
      }
      j++;
    }
...
void HM_Display::setup_wifi() {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    u8g2.setFont(u8g2_font_courB08_tf);
    u8g2.clearBuffer();
    int i = 0;
    while(i < numItems) {
      if (i == pos and i < numItems-1) {
        u8g2.setCursor(0, (pos + 1) * y_offset + 2);
        u8g2.print("*");
      }
      else if (i == pos and i == numItems-1) {
    ...
    while(i < numItems) {
      if (i < numItems - 1) {
        ...
        if (i == pos and change_value == false) {
          u8g2.setCursor(1, 2+((i+1) * y_offset));
          u8g2.print("*");
        }
        else if (i == pos and change_value == true) {
          u8g2.setCursor(1, ((i+1) * y_offset));
          u8g2.print("-");
        }
      }
      else {
...
void HM_Display::setup_max_weight() {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    u8g2.setFont(u8g2_font_courB08_tf);
    u8g2.clearBuffer();
    int i = 0;
    while(i < numItems) {
      if (i < numItems - 1) {
        ...
        u8g2.print(menuitems[i]);
        switch (i) {
          case 0: sprintf(output,"%4dKg", max_lc_weight / 1000);
                  u8g2.setCursor(90, (i+1) * y_offset);
                  u8g2.print(output);
                  break;
          case 1:
                  if (max_lc_cal < 1000) {
                    sprintf(output,"%5dg", max_lc_cal);
                  }
                  else {
                    sprintf(output,"%4dKg", max_lc_cal / 1000);
                  }
                  u8g2.setCursor(90, (i+1) * y_offset);
                  u8g2.print(output);
                  break;
  ...
  while(j < numItems) {
    ...
    if (j < numItems-1) {
      ...
      tft.setTextColor(BACKGROUND);
      switch (j) {
        case 0: sprintf(output,"%dKg", max_lc_weight / 1000);
                if (value_old != max_lc_weight and change_value == true and j == pos) {
                  sprintf(del_text,"%dKg", value_old / 1000);
                  tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                  tft.print(del_text);
                  value_old = max_lc_weight;
                }
                break;
        case 1: if (max_lc_cal < 1000) {sprintf(output,"%dg", max_lc_cal);}
                else {sprintf(output,"%dKg", max_lc_cal/1000);}
                if (value_old != max_lc_cal and change_value == true and j == pos) {
                  if (value_old < 1000) {sprintf(del_text,"%dg", value_old);}
                  else {sprintf(del_text,"%dKg", value_old/1000);}
                  tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                  tft.print(del_text);
                  value_old = max_lc_cal;
                }
                break;
      }
      if (j == pos) {
      ...
      tft.setCursor(tft.width() - 5 - tft.textWidth(output), 30+((j+1) * y_offset_tft));
      tft.print(output);
    }
    //if (j < numItems - 1) {
    //  tft.setCursor(5, 30+((j+1) * y_offset_tft));
...
void HM_Display::setup_counter(int a) {
  int j;
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 1) { //screen 1
      j = 0;
...
void HM_Display::setup_trip_counter(int a) {
  int j;
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 1) { //screen 1
      j = 0;
...
void HM_Display::setup_fuellmenge(int a) {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 1) {
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
...
void HM_Display::setup_calibration(int a) {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 1){
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
...
void HM_Display::setup_INA219(int a) {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a ==1) {   //main menu
      u8g2.setFont(u8g2_font_courB08_tf);
      u8g2.clearBuffer();
    ...
    if (a ==2) {
      ...
      if (change_value == false && menuitem < numItems && calibration_status == START[lingo]) {
    ...
    if (a ==1) {   //main menu
      ...
      while(j < numItems) {
        ...
        if (j < numItems-1) {
          if (!change_value or change) {
            tft.setCursor(5, 27+((j+1) * y_offset_tft));
            tft.print(menuitems[j]);
          }
          if (change and j == numItems-2) {
            text_old = menuitems[1];
            change = false;
          }
          tft.setTextColor(BACKGROUND);
          switch (j) {
            case 0: if (current_servo > 0) {
                    ...
                    if (value_old != current_servo and change_value == true and j == pos) {
              ...
              case 1: if (current_servo == 0) {
                        ...
                        if (value_old != show_current and change_value == true and j == pos) {
...
void HM_Display::setup_webif(int i) {
  #if DISPLAY_TYPE == 991 || DISPLAY_TYPE == 992 || DISPLAY_TYPE == 999
...
void HM_Display::onlineOTA(int i) {
  #if DISPLAY_TYPE == 991 || DISPLAY_TYPE == 992 || DISPLAY_TYPE == 999
    if (i == 1 || i == 2 || 3) { //Start WiFi
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB14_tf);
      sprintf(output, START_WIFI[lingo]);
      x_pos = CenterPosX(output, 11, 128);
      u8g2.setCursor(x_pos, 25);
      u8g2.print(output);
      if (i == 1) {
    ...
    if (i == 4) { //Start Update
      sprintf(output, START_UPDATE[lingo]);
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB14_tf);
      x_pos = CenterPosX(output, 11, 128);
      u8g2.setCursor(x_pos, 25);
    ...
    if (i == 5) { //Kein Update erforderlich
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
      sprintf(output, FIRMWARE_IS[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 15);
      u8g2.print(output);
      sprintf(output, UP_TO_DATE[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 25);
      u8g2.print(output);
      sprintf(output, NO_UPDATE[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 35);
      u8g2.print(output);
      sprintf(output, REQUIRED[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 45);
      u8g2.print(output);
      u8g2.sendBuffer();
    }
    if (i == 6) { //Update progress
      sprintf(output, START_UPDATE[lingo]);
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB14_tf);
      x_pos = CenterPosX(output, 11, 128);
    ...
    if (i == 7) { //Update erforlgreich
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
      sprintf(output, SUCCESSFULLY[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 35);
      u8g2.print(output);
      u8g2.sendBuffer();
    ...
    if (i == 8) { //Update Error
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
      sprintf(output, ERROR[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 35);
      u8g2.print(output);
      u8g2.sendBuffer();
    }
    if (i == 9) { //kein .bin Filegefunden
      u8g2.clearBuffer();
      sprintf(output, NO_BIN_FILE[lingo]);
      u8g2.setFont(u8g2_font_courB08_tf);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 30);
      u8g2.print(output);
      sprintf(output, AVARILABLE[lingo]);
      x_pos = CenterPosX(output, 6, 128);
      u8g2.setCursor(x_pos, 40);
      u8g2.print(output);
      u8g2.sendBuffer();
    }
    if (i == 10 || i == 11 || i == 12) { //Update Errors
      u8g2.clearBuffer();
      sprintf(output, START_UPDATE[lingo]);
      u8g2.setFont(u8g2_font_courB14_tf);
      x_pos = CenterPosX(output, 11, 128);
      u8g2.setCursor(x_pos, 25);
      u8g2.print(output);
      sprintf(output, "%s: %d", FAILED[lingo], i);
      x_pos = CenterPosX(output, 11, 128);
      u8g2.setCursor(x_pos, 45);
      u8g2.print(output);
      u8g2.sendBuffer();
    }
    if (i == 13) { //Fragen ob FW-Uptate gestartet werden soll
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
      u8g2.setCursor(5, 10);
      sprintf(output, "%s:", OLD_VERSION[lingo]);
      u8g2.print(output);
      u8g2.setCursor(5, 20);
      sprintf(output, "%s:", NEW_VERSION[lingo]);
      u8g2.print(output);
      u8g2.setCursor(78, 10);
      sprintf(output,"%8s", VERSION_STRING);
      u8g2.print(output);
      u8g2.setCursor(78, 20);
      sprintf(output,"%8s", new_version);
      u8g2.print(output);
      u8g2.setCursor(5, 40);
      u8g2.print(BUTTONS[lingo]);
      u8g2.setCursor(5, 50);
      sprintf(output,"%s:", START[lingo]);
      u8g2.print(output);
      u8g2.setCursor(5, 60);
      sprintf(output,"%s:", STOP[lingo]);
      u8g2.print(output);
      u8g2.setCursor(78, 50);
      sprintf(output,"%8s", UPDATE[lingo]);
      u8g2.print(output);
      u8g2.setCursor(78, 60);
      sprintf(output,"%8s", EXIT[lingo]);
      u8g2.print(output);
      u8g2.sendBuffer();
    }
    if (i > 100) { //Failed
    ...
    if (i == 1) { //Draw Frame
      ...
      tft.setTextColor(TEXT);
      tft.setFreeFont(F16B[font_typ]);
      sprintf(output, "%s", OTA_ONLINE[lingo]);
      calculateStringMetrics(F16B[font_typ], "MC", output, tft.width()/2, 15, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(output);
      tft.drawLine(0, 32, tft.width(), 32, TEXT);
    ...
    if (i == 5) { //Kein Update erforderlich
      ...
      tft.setTextColor(TFT_GREEN);
      tft.setFreeFont(F18B[font_typ]);
      calculateStringMetrics(F18B[font_typ], "MC", FIRMWARE_IS[lingo], tft.width()/2, 80, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(FIRMWARE_IS[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", UP_TO_DATE[lingo], tft.width()/2, 120, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(UP_TO_DATE[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", NO_UPDATE[lingo], tft.width()/2, 160, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(NO_UPDATE[lingo]);
      calculateStringMetrics(F18B[font_typ], "MC", REQUIRED[lingo], tft.width()/2, 200, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(REQUIRED[lingo]);
      tft.setTextColor(TEXT);
    }
    if (i == 6) { //Update progress
      float a = 0.7;
    ...
    if (i == 13) { //Fragen ob FW-Uptate gestartet werden soll
      ...
      tft.setCursor(5, 30+(2 * y_offset_tft));
      tft.print(output);
      sprintf(output, "%s:", BUTTONS[lingo]);
      tft.setCursor(5, 30+(5 * y_offset_tft));
      tft.print(output);
      sprintf(output, "%s:", START[lingo]);
      tft.setCursor(5, 30+(6 * y_offset_tft));
      tft.print(output);
      sprintf(output, "%s:", STOP[lingo]);
      tft.setCursor(5, 30+(7 * y_offset_tft));
      tft.print(output);
      sprintf(output,"%s", VERSION_STRING);
      calculateStringMetrics(F12B[font_typ], "BR", output, tft.width() - 5, 30+(1 * y_offset_tft), x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(output);
      sprintf(output,"%s", new_version);
      calculateStringMetrics(F12B[font_typ], "BR", output, tft.width() - 5, 30+(2 * y_offset_tft), x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(output);
      sprintf(output,"%s", UPDATE[lingo]);
      calculateStringMetrics(F12B[font_typ], "BR", output, tft.width() - 5, 30+(6 * y_offset_tft), x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(output);
      sprintf(output,"%s", EXIT[lingo]);
      calculateStringMetrics(F12B[font_typ], "BR", output, tft.width() - 5, 30+(7 * y_offset_tft), x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
      tft.print(output);
    }
    if (i > 100) { //Failed
...
void HM_Display::ota_setup(int a) {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 1) {
      u8g2.clear();
    }
    ...
    if (a == 6) {
      ...
      u8g2.setCursor(1, 25);
      sprintf(output, "Current: %u", ota_current);
      u8g2.print(output);
      u8g2.setCursor(1, 35);
      sprintf(output, "Final:   %u", ota_final);
      u8g2.print(output);
      u8g2.sendBuffer();
    }
    if (a == 7) {
    ...
    if (a == 2) {
      ...
      tft.print(del_text_1);
      tft.setTextColor(TEXT);
      calculateStringMetrics(F20B[font_typ], "MC", stars, tft.width()/2, 136, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] + x_y_h_w_b[2] * a);
      tft.print(stars);
      sprintf(del_text_1,"%s", stars);
    }
    if (a == 3) {
      tft.fillRect(0, 33, tft.width(), tft.height()-33, BACKGROUND);
      tft.setTextColor(TEXT);
    ...
    if (a == 6) {
      ...
      tft.print(del_text_2);
      tft.setTextColor(TEXT);
      sprintf(del_text_1, "%u", ota_current);
      sprintf(del_text_2, "%u", ota_final);
      calculateStringMetrics(F14B[font_typ], "BL", del_text_1, longest_string_1 + 20, 136, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - x_y_h_w_b[2] * a);
      tft.print(del_text_1);
      calculateStringMetrics(F14B[font_typ], "BL", del_text_2, longest_string_1 + 20, 136, x_y_h_w_b);
...
void HM_Display::setup_turntable(int a) {
  #if DISPLAY_TYPE == 991 or DISPLAY_TYPE == 992 or DISPLAY_TYPE == 999
    if (a == 0) {
      u8g2.clearBuffer();
      u8g2.sendBuffer();
    }
    if (a == 1) {                                  //No connection
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_courB08_tf);
    ...
    if (a == 2) {                                  //Main Menu
      ...
      while(i < numItems) {
        if (i < numItems-1) {
          ...
          u8g2.print(menuitems[i]);
          switch (i) {
            case 0: sprintf(output,"%3s", (use_turntable==0?OFF[lingo]:ON[lingo]));
                    u8g2.setCursor(110, (i+1) * y_offset);
                    u8g2.print(output);
                    break;
            case 1: if (use_turntable == 1 and turntable_init == false) {
    ...
    if (a == 3) {                                  //Menu Setup Turntable
      ...
      while(i < numItems) {
        if (i < numItems-1) {
          ...
          u8g2.print(menuitems[i]);
          switch (i) {
            case 0: sprintf(output,"%4s", esp_now_wait);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
            case 1: sprintf(output,"%4i", jar_center_pos);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
            case 2: sprintf(output,"%4i", speed_init);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
            case 3: sprintf(output,"%4i", speed_run);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
          }
        ...
        if (i < numItems-1) {
          ...
          switch (i) {
            ...
            case 1: sprintf(output,"");
                    break;
            case 2: sprintf(output,"%4i", ts_speed);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
            case 3: sprintf(output,"%4i", ts_waittime);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
            case 4: sprintf(output,"%4i", ts_angle_min);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
            case 5: sprintf(output,"%4i", ts_angle_max);
                    u8g2.setCursor(105, (1+i) * y_offset);
                    u8g2.print(output);
                    break;
          }
    ...
    if (a == 1) {                                  //No connection
      ...
      tft.setFreeFont(F20B[font_typ]);
      sprintf(output, "%s", CONNECTION[lingo]);
      calculateStringMetrics(F20B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
      tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1] - x_y_h_w_b[2]/2 - x_y_h_w_b[2] * a);
      tft.print(output);
      sprintf(output, "%s", FAILED[lingo]);
      calculateStringMetrics(F20B[font_typ], "MC", output, tft.width() / 2, tft.height() / 2, x_y_h_w_b);
    ...
    if (a == 2) {                                  //Main menu
      ...
      tft.setFreeFont(F12B[font_typ]);
      if (clear_tft > 0) {
        if      (clear_tft == 1) {tft.fillRect(0, 65, 320, 142, BACKGROUND);}
        else if (clear_tft == 2) {tft.fillRect(0, 35, 320, 205, BACKGROUND);}
        clear_tft = 0;
      }
      while(j < numItems) {
        tft.setTextColor(TEXT);
        ...
        if (j < numItems - 1) {
          ...
          sprintf(output,"");
          switch (j) {
            case 0: sprintf(output,"%s", use_turntable==false?OFF[lingo]:ON[lingo]);
                    if (j == pos and value_old != use_turntable) {
                      del_text = use_turntable==true?OFF[lingo]:ON[lingo];
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = use_turntable;
                    }
                    break;
            case 1: sprintf(output,"");
                    if (use_turntable == 1 and turntable_init == false) {
                      if (turntable_running == true) {
                        ...
                        if (esp_now_wait != esp_now_wait_old) {
                          sprintf(output,"%s", esp_now_wait_old);
                          tft.setCursor(tft.width() - 5 - tft.textWidth(output), 30+((j+1) * y_offset_tft));
                          tft.print(output);
                          esp_now_wait_old = esp_now_wait;
                        }
                        sprintf(output,"%s", esp_now_wait);
                        esp_now_wait_old = esp_now_wait;
                      }
                      else {
                        sprintf(output,"%s", turntable_init==false?NOKAY[lingo]:OKKAY[lingo]);
                      }
                    }
                    break;
          }
    ...
    if (a == 3) {                                  //Menu Setup Turntable
      ...
      tft.setFreeFont(F12B[font_typ]);
      if (clear_tft == 1) {
        tft.fillRect(0, 35, 320, 172, BACKGROUND);
        clear_tft = 0;
      }
      while(j < numItems) {
        tft.setTextColor(TEXT);
        ...
        if (j < numItems - 1) {
          ...
          switch (j) {
            case 0: 
                    break;
            case 1: sprintf(output,"%i", jar_center_pos);
                    if (j == pos and value_old != jar_center_pos) {
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = jar_center_pos;
                    }
                    break;
            case 2: sprintf(output,"%i", speed_init);
                    if (value_old != speed_init and change_value == true and j == pos) {
                      change = true;
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = speed_init;
                    }
                    break;
            case 3: sprintf(output,"%i", speed_run);
                    if (value_old != speed_run and change_value == true and j == pos) {
                      change = true;
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
    ...
    if (a == 4) {                                  //Menu Setup Dropprodection
      String del_text = "";
      int j = 0;
      tft.setFreeFont(F12B[font_typ]);
      if (clear_tft == 1) {
      ...
      while(j < numItems) {
        ...
        if (j < numItems - 1) {
          ...
          switch (j) {
            ...
            case 1: sprintf(output,"");
                    break;
            case 2: sprintf(output,"%i", ts_speed);
                    if (j == pos and value_old != ts_speed) {
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = ts_speed;
                    ...
                    break;
            case 3: sprintf(output,"%i", ts_waittime);
                    if (j == pos and value_old != ts_waittime) {
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = ts_waittime;
                    }
                    break;
            case 4: sprintf(output,"%i", ts_angle_min);
                    if (j == pos and value_old != ts_angle_min) {
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = ts_angle_min;
                    }
                    break;
            case 5: sprintf(output,"%i", ts_angle_max);
                    if (j == pos and value_old != ts_angle_max) {
                      del_text = value_old;
                      tft.setCursor(tft.width() - 5 - tft.textWidth(del_text), 30+((j+1) * y_offset_tft));
                      tft.print(del_text);
                      value_old = ts_angle_max;
                    }
                    break;
          }
          if (j == pos) {
    ...
    if (a == 5) {                                  //Menu OTA Update
      if (ota_status == 0) {
        tft.fillRect(0, 35, 320, 205, BACKGROUND);
        tft.setTextColor(TEXT);
        tft.setFreeFont(F26B[font_typ]);
        sprintf(output, "Start WiFi");
        calculateStringMetrics(F26B[font_typ], "MC", output, tft.width()/2, 136, x_y_h_w_b);
        tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
        tft.print(output);
      }
      if (ota_status == 1) {
        ...
        tft.setFreeFont(F18B[font_typ]);
        sprintf(output, myReceivedMessage_text);
        calculateStringMetrics(F18B[font_typ], "MC", output, tft.width()/2, 136, x_y_h_w_b);
        tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
        tft.print(output);
      }
      if (ota_status == 2) {
        ...
        tft.setFreeFont(F20B[font_typ]);
        sprintf(output, "Start WiFi");
        calculateStringMetrics(F20B[font_typ], "MC", output, tft.width()/2, 136-24, x_y_h_w_b);
        tft.setCursor(x_y_h_w_b[0], x_y_h_w_b[1]);
        tft.print(output);
        sprintf(output, "failed");
        calculateStringMetrics(F20B[font_typ], "MC", output, tft.width()/2, 136+24, x_y_h_w_b);
      ...
      if (ota_status == 3) {
        ...
        tft.print("OTA Progress:");
        tft.setCursor(10, 140);
        tft.print("Current:");
        tft.setCursor(10, 170);
        tft.print("Final:");
        tft.setTextColor(BACKGROUND);
        if (strcmp(myReceivedMessage_text, del_text_1) != 0) {
          tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_1), 140);
          sprintf(output, "%s", del_text_1);
          tft.print(output);
        }
        if (myReceivedMessage_value != atoi(del_text_2)) {
          tft.setCursor(tft.width() - 5 - tft.textWidth(del_text_2), 170);
          sprintf(output, "%s", del_text_2);
          tft.print(output);
        }
        tft.setTextColor(TEXT);
        tft.setCursor(tft.width() - 5 - tft.textWidth(myReceivedMessage_text), 140);
        sprintf(output, "%s", myReceivedMessage_text);
        tft.print(output);
        tft.setCursor(tft.width() - 5 - tft.textWidth(String(myReceivedMessage_value).c_str()), 170);
        sprintf(output, "%i", myReceivedMessage_value);
        tft.print(output);
        sprintf(del_text_1, "%s", myReceivedMessage_text);
        sprintf(del_text_2, "%s", String(myReceivedMessage_value).c_str());
      }
      if (ota_status == 4) {
        tft.fillRect(0, 35, tft.width(), tft.height()-35, BACKGROUND);
        tft.setTextColor(TFT_RED);
        tft.setFreeFont(F20B[font_typ]);
        sprintf(output, "Error");
        calculateStringMetrics(F20B[font_typ], "MC", output, tft.width()/2, 136, x_y_h_w_b);
